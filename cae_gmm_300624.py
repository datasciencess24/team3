...
RECORDING_ANOMALY_THRESHOLD = 99.99  # Threshold for recording anomaly (beyond 99.99th percentile = anomalous)
    ...
    for root, dirs, files in os.walk(directory):
        if 'raw' in dirs:  # Check if 'raw' is a direct child of the current directory
            ...
            if os.path.exists(ae_file) and os.path.exists(ec_file) and "OK" in root:
                test_files.append((parent_dir, ae_file, ec_file))  # Use parent_dir instead of root
...
def extract_features_and_train(train_files, segment_duration_ms):
    ...
    # Save trained model
    model_path = '/home/dsbwl24_team003/data/temp/cae_gmm_model.pkl'
    joblib.dump((autoencoder, gmm, threshold), model_path)
...
# Before training loop, move model to GPU
autoencoder = ConvAutoencoder().to(device)
...
np.save('/home/dsbwl24_team003/data/temp/Test_202402-6/normal_proportions.npy', normal_proportions)
...
for root, ae_file, ec_file in test_files:
    proportion_anomalous = detect_anomalies(autoencoder, gmm, threshold, ae_file, ec_file, segment_duration_ms)
    if proportion_anomalous is None:
        continue

    # Extract folder name containing 'raw'
    folder_name = root.split('/')[-1] if '/' in root else root.split('\\')[-1]
    
    is_anomalous = flag_recording_as_anomalous(proportion_anomalous, normal_proportions)
    results[folder_name] = 'anomalous' if is_anomalous else 'normal'
    print(f"Recording {folder_name}: {results[folder_name]}")
